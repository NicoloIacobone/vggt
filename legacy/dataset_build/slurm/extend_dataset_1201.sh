#!/bin/bash
# Dataset extension to the full official ScanNet v2 train split (docs/todo.md 1c):
# for scenes [list_start..list_end] (0-based, inclusive indices into
# data/splits/scannetv2_train.txt, 1201 scenes total), stream the .sens to extract the
# stride-5 subset frames (legacy/dataset_build/scripts/extract_sens_subset.py, early-abort
# streaming — no .sens ever touches disk), then download the official 2D GT zips and
# convert them into the build tree (legacy/dataset_build/scripts/download_2d_gt.py, zips
# deleted per scene).
#
# NODE-LOCAL BUILD (docs/DATASET.md §6). The build tree is ~1046 files per scene — 1.26M
# inodes for the full split, against a 1.0M soft / 1.5M hard FILE quota on scratch. The
# first attempt built on scratch directly and died with OSError(122, 'Disk quota
# exceeded') at 1090/1201 scenes. So the tree is built in $TMPDIR (node-local SSD, wiped
# when the job ends, no quota) and only ONE compressed tar per chunk ever lands on
# scratch. Scratch has 2.4 TB of block quota and is barely using it — bytes are free,
# inodes are not.
#
...
#     and the job then exits non-zero so --dependency=afterok holds the packing step back.
#
# The official train split is NOT a contiguous scene0000_00.. range (includes _01/_02/...
...
#
# Nothing here touches the 500-scene tree/tar (scannet_official_gt_500.tar.zst on work)
# that other runs are training against.
#
# Submit in parallel chunks, e.g. (1201 scenes / 4 chunks of ~300):
#   sbatch legacy/dataset_build/slurm/extend_dataset_1201.sh    0  299
#   sbatch legacy/dataset_build/slurm/extend_dataset_1201.sh  300  599
#   sbatch legacy/dataset_build/slurm/extend_dataset_1201.sh  600  899
#   sbatch legacy/dataset_build/slurm/extend_dataset_1201.sh  900 1200
# After all finish: QA + pack via legacy/dataset_build/slurm/pack_official_gt_1201.sh
# (EXPECT_SCENES=1201), e.g. with --dependency=afterok:<job1>:<job2>:<job3>:<job4>.
#
#SBATCH --job-name=extend_gt_1201
#SBATCH --output=/cluster/scratch/niacobone/vggt/slurm/logs/extend_gt_1201_%j.log
#SBATCH --error=/cluster/scratch/niacobone/vggt/slurm/logs/extend_gt_1201_%j.err
...
SCENE_LIST=$REPO/data/splits/scannetv2_train.txt
...
# Node-local build tree. --tmp=120000 covers: restored tar read (streamed, not copied) +
# unpacked tree (~31 GB at 1201 scenes) + transient zips + the output tar (~24 GB).
...
if [ "$n_done" -lt "$n_want" ]; then
    ...
    if [ "${RESUBMIT:-1}" != "1" ]; then
        ...
        sbatch --export=ALL,RESUBMIT_N=$n_try \
            legacy/dataset_build/slurm/extend_dataset_1201.sh "$LIST_START" "$LIST_END"
    fi
    exit 1
fi
...
if [ "${CHAIN_PACK:-0}" = "1" ]; then
    echo "[extend] CHAIN_PACK=1 — submitting pack_official_gt_1201.sh"
    sbatch legacy/dataset_build/slurm/pack_official_gt_1201.sh
fi
