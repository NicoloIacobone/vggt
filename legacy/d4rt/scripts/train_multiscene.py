...
from legacy.d4rt.train.loss import D4RTLoss
from train.eval_metrics import compute_instance_segmentation_metrics
from train.common import (DEFAULT_SCANS_ROOT, append_jsonl, build_scheduler,
                          photometric_jitter, resolve_scene_dirs)
from data.scannet_overfit import ScanNetMultiSceneDataset
from legacy.d4rt.models.anchor_queries import build_anchors, jitter_anchors
    ...
    if args.save_checkpoint and not args.no_visualize:
        ...
        try:
            ...
            print(f"⚠ Visualization failed ({e}). Render manually with:\n"
                  f"  python legacy/d4rt/scripts/visualize_masks.py --checkpoint {vis_ckpt}")
