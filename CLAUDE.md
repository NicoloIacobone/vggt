...
This is a fork of **VGGT** (Visual Geometry Grounded Transformer, CVPR 2025) — a feed-forward 3D reconstruction model. The project goal is **not** to modify VGGT itself, but to attach and train a **D4RT-style / DETR-like decoder for 3D multi-view consistent segmentation** on top of the frozen VGGT-1B backbone. Ground-truth supervision is the **official ScanNet v2 2D instance annotations** (default since 2026-07-08; see Storage layout). The previous GT — masks from running **SAM3 on ScanNet scenes** — was found to have systematic cross-class duplicates (audit 2026-07-07: ~15.9% multi-class foreground pixels, the same object labeled under two classes → built-in honest-AP50 false positives) and is kept only as the baseline for GT-quality comparisons; migration record: `docs/old/OFFICIAL_GT_MIGRATION_PLAN.md`. The loader defaults to per-class, `--instance_level` switches to per-instance.
...
# Quotable baseline (official GT, 2026-07-08): val mIoU 0.367, honest val[grid] AP50 0.199
# (run d4rt_full_inst_learned_officialgt_20260708_124452; the older SAM3-GT numbers
# 0.371/0.228 don't survive clean GT — see docs/MILESTONES.md §Dataset status). For the
...
# Official-GT dataset rebuild (only if the tar is lost/needs changes; docs/old/OFFICIAL_GT_MIGRATION_PLAN.md):
...
- **Datasets ship ONLY as tars on work** (`/cluster/work/igp_psr/niacobone/distillation/dataset/scannet/`); there is NO unpacked `scans/` tree on work (deliberately deleted 2026-07: reading thousands of small PNGs off `work` is slow and pressures the inode quota). Three tars, identical layout `scans/<scene>/raw_data/{subset,masks,masks_instance,_qa}`, each scene with `subset/` (~100 stride-5 frames), `masks/<class>/` (per-class binary), `masks_instance/<class>_<k>/` (per-instance binary):
  - **`scannet_official_gt_500.tar.zst` (the DEFAULT since 2026-07-09)** — official ScanNet v2 GT, **500 scenes (scene0000–0499), 7379 instances, 0 cross-class duplicates**. Scenes 0200–0499 built 2026-07-09 (`slurm/extend_dataset_500.sh`: subsets streamed straight out of each scene's `.sens` via `scripts/extract_sens_subset.py` — early-abort streaming, no .sens stored). 9 scenes (0240, 0243, 0269, 0292, 0354, 0366, 0438, 0456, 0483) have a **640×480** color camera; their GT is 640×480 too (RGB↔GT consistent; loader resizes to 518).
  - `scannet_official_gt_full.tar.zst` (2.3 GB) — the original 200-scene official-GT tar (scene0000–0199, 2950 instances); kept for exact reproducibility of the runs trained on it. Both official tars: one class per object, cross-view-consistent ids, sparse masks (missing file = instance not visible; the loader skips), stuff classes keep official ids (`wall_0..wall_k`). Spec: `…/scannet/OFFICIAL_GT_README.md`. Built by `scripts/build_official_masks.py` from the official `_2d-instance-filt`/`_2d-label-filt` zips (rebuild: `slurm/download_official_gt.sh` / `slurm/extend_dataset_500.sh` + `slurm/pack_official_gt.sh`).
  - `scannet_instance_dataset_full.tar.zst` (~2.6 GB) — the SAM3-generated GT (200 scenes, ≈4195 instances, but ~3.4 cross-class duplicate instances/scene; audit 2026-07-07). Kept as baseline + project deliverable. Cross-frame identity from SAM3 video tracking; `wall`/`floor` forced single-instance; all-zero PNGs written where absent. Spec: `…/scannet/INSTANCE_MASKS_README.md` (+`…_split2.md`).
- **Dataset access at training time:** each job copies ONE tar to node-local scratch (`$TMPDIR`) and unpacks it there (`slurm/stage_dataset.sh`, exports `SCANNET_ROOT=$TMPDIR/scans`; `scripts/train_multiscene.py` uses `SCANNET_ROOT` as default `--scans_root`). The tar is chosen by `DATA_TAR`: stage_dataset.sh's own default is still the SAM3 tar (backward compat), but **every train SLURM script overrides it to the 500-scene official tar** and requests `#SBATCH --tmp=24000` (MB, covers the 500-scene tar + unpacked tree). `zstd` at `/usr/bin/zstd`. The official-GT build tree currently also exists unpacked at `/cluster/scratch/niacobone/scannet_official_build/scans` (scratch — purgeable; the tars on work are canonical). The old SAM3 scratch build trees (`scannet_build*`) are hollow (purge ate the PNGs) — re-pack from the tar if ever needed, never unpack-to-retar on scratch (inode quota).
- **Mask conventions (both GTs):** uint8 {0,255}, resolution = the scene's color camera (1296×968; 640×480 for the 9 scenes above), filename = subset stem + `.png`; `<k>` zero-based per class in order of first appearance; dir names use underscores (`shower_curtain_3`). Union of a class's instance masks = its `masks/<class>/` mask. The loader defaults to per-class `masks/`; `--instance_level` switches to per-instance (`data/scannet_overfit.py::instance_level`). In official GT, NYU40 classes outside the 19 trainable (incl. `otherfurniture`) are background — the class head's 20 logits can't represent index 20.
