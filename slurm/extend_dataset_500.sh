...
# stream the .sens to extract the stride-5 subset frames (scripts/extract_sens_subset.py,
# early-abort streaming — no .sens ever touches disk), then download the official
# 2D GT zips and convert them into the build tree (scripts/download_2d_gt.py,
...
#
# Submit two parallel ranges: sbatch slurm/extend_dataset_500.sh 200 349
#                             sbatch slurm/extend_dataset_500.sh 350 499
# After both finish: QA + pack via slurm/pack_official_gt.sh (EXPECT_SCENES=500).
#
#SBATCH --job-name=extend_gt_500
#SBATCH --output=extend_gt_500_%j.log
#SBATCH --error=extend_gt_500_%j.err
...
myenv/bin/python scripts/extract_sens_subset.py \
